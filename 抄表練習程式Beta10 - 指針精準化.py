...
answer_no = [1] #作答序號
anstime_set=[0]
anstime_set[0]=12 # <----倒數秒數設定
anstime=[0]
anstime[0]=anstime_set[0] #作答計時
...
answerWrite.write("\n")
answerWrite.write("序號  狀態        題目          答案            對錯題數  答題時間  思考秒數\n")
answerWrite.close()
...
def inx(index):
    ...
    nex[0]=index[0]
    nex[1]=36* ((index[1]*1000) + (index[2]*100) + (index[3]*10)+ index[4]) / 10000
    nex[2]=36* ((index[2]*100) + (index[3]*10)+ index[4]) /1000
    nex[3]=36* ((index[3]*10)+ index[4]) /100
    nex[4]=36* (index[4]*1) /10
    
    for i in range(5):
        if(i%2==0):
            ans = cw[index[i]]
            if i<4:ans = ans+(nex[i+1])
...
def pointer(degree):  # array input!
    ...
    for c in range(5):
        ...
        pp_size = int(rad/72) # pointer tail width size
...
# def 初始化
def initial():
    main_window.title("台灣電力公司  - 抄 表 練 習 程 式 Beta10(指針精準化) by Ming")
    main_window.geometry('800x320')
    label = tk.Label(main_window,text="抄 表 練 習 程 式 Beta10(指針精準化)",bg = 'SpringGreen3',font=('Bahnschrift Bold',13)).place(x=2,y=3)
    circle_frame(Ox,Oy)
    central_dot(Ox,Oy,"gold")
...
def clock(): #小時鐘
    practice_time[0]=practice_time[0]+1
    t=time.strftime('%H:%M:%S',time.localtime())
    if t!='':clock_label.config(text=t,font='times 18')
    main_window.after(1000,clock)

def timer(): #測驗計時器
    anstime[0]=anstime[0]-1
    if(anstime[0]==0):
        anstime[0]=anstime_set[0]
...
def input_index(event): #檢查指數合法
    ...
    display_label.set(input_index)
    if(is_number(input_index)and len(input_index)==5):
        check(input_index)
    else:
        display_label.set('指數無效!')
        show_String = tk.Label(main_window,textvariable = display_label, bg='yellow',font = "Helvetica 24 bold",justify="center")
        show_String.place(x=350,y=210)
        text_box.delete(0,END)

def check(index): #核對指數
    answer_no[0] = answer_no[0]+1
    answer_list[2],answer_list[3] = index2,ans
    answerWrite=open(LocalDirectory +"/" + log ,"a")
    answerWrite.write('\n')
    second = answer_time()
    second = str(second)+'s'
    ...
    if(setting[2]):
        display_label.set(switch_HC(list(str(index))))
        b=switch_HC(b)
    for i in range(5):
        ans[i]=int(b[i])

    if(correct_count[0]%10==0):display_label.set("您已抄"+str(correct_count[0])+"顆! 錯誤"+str(error_count[0])+"顆!")
    timestamp=time.strftime('%H:%M:%S',time.localtime())
    print(ans,"<-你的答案")
    if(ans==index2):
        anstime[0]=anstime_set[0]
        show_String = tk.Label(main_window,textvariable = display_label, bg='OliveDrab2',font = "Helvetica 24 bold",justify="center")
        answer_list[1]="PASS!"
        print(answer_list,"correct",correct_count,second,'\n')
        answerWrite.write(str(answer_list)+str('   PASS')+str(correct_count)+" "+timestamp+"  "+second)
        correct_count[0]=correct_count[0]+1
        start()
    else:
        show_String = tk.Label(main_window,textvariable = display_label, bg='firebrick1',font = "Helvetica 24 bold",justify="center")
        if(answer_list[1]!="ERROR"):error_count[0]=error_count[0]+1
        answer_list[1]="ERROR"
        print(answer_list,"wrong",error_count,second,'\n')
        answerWrite.write(str(answer_list)+str('   ERROR')+str(error_count)+" "+timestamp+"  "+second+str('  <-----X'))
...
def answer_time():  #計算作答時間
    t= practice_time[0]-think_time[0]
    think_time[0]=practice_time[0]
    return t
...
def show_you_around():
    ...
    show_around=index2 #就地起跑!
    for j in range(5):
        index2[j]=int(show_around[j])
        #print(show_around)
        clear()
        pointer(inx(index2))
    ...
    a = ''
    for i in range(5):
        a = a+ str(show_around[i])
    a = int(a)
    a = a+1;
    a = str(a).zfill(5)
    for i in range(5):
        show_around[i] = int(a[i])

    if(showdown[0]!=0):
        main_window.after(100,show_you_around)
        showdown[0]=showdown[0]-1
        #print(showdown)
        display_label.set(show_around)
        show_String = tk.Label(main_window,textvariable = display_label, bg='yellow',font = "Helvetica 24 bold",justify="center")
    else:
        showdown[0]=500
...
show_you_around_btn = tk.Button(main_window,text="Run!",command=show_you_around , height=1, width=5, font="Helvetica 10 bold",justify="center",bg='orange')
start_btn.place(x=220,y=215)
show_btn.place(x=220,y=285)
show_you_around_btn.place(x=160,y=285)


rad_ratio=tk.Scale(main_window , from_=72,to=24 ,orient = VERTICAL, length=60 ,label='size' ,resolution = 3,command = ratio_mod)
rad_ratio.place(x=150,y=210)
